...
try {
  ...
  console.log('R2/R3 drain stepdown: zero loss, read-only while draining, fast promotion');
  {
    ...
    ok(!w || (w.status === 503 && wBody.reason === 'draining'), `a write during the drain is refused 503 draining (got ${w ? w.status : 'connection closed'})`);
    const nl = await f.waitSingleLeader({ timeoutMs: 60000, minEpoch: 2 });
  ...
  else console.log('✅ replication.test: all assertions passed (REPLICATE_MS honoured, loss-free drain stepdown, read-only drain, 3-node race → one leader, no-replica drain does not hang)');
} catch (e) {
  failed = true;
