...
//   C3  a NEW DM gets a re-issued id (≤ the lane's old cursor) — and the lane RECEIVES it
...
try {
  ...
  console.log('C1–C4 lossy failover re-issues ids below the lane cursor');
  {
    ...
    ok(count(rx, /^N1 first/) === 1 && count(rx, /^N2 second/) === 1, `C4 exactly once (N1×${count(rx, /^N1 first/)}, N2×${count(rx, /^N2 second/)})`);
