...
try {
  ...
  let out = '', err = '';
  poll = spawn(process.execPath, [POLL, 'otherbox/Foo_Bar', '--base', BASE, '--token', TOKEN], {
    env: { ...process.env, HOME, USERPROFILE: HOME }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true,
  });
  poll.stdout.on('data', (d) => { out += d; });
  poll.stderr.on('data', (d) => { err += d; });
  for (let end = Date.now() + 20000; Date.now() < end && !/\[listening as/.test(err); ) await sleep(100);
  ok(/\[listening as/.test(err), 'cc-poll is listening (cursors seeded)');
  ...
  for (let end = Date.now() + 15000; Date.now() < end && !(/A dm/.test(out) && /B hey/.test(out) && /C @all/.test(out)); ) await sleep(200);
